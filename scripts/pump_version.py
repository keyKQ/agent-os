...
def bump_releases_md(
    repo_root: Path, new_version: str, new_tag: str, release_date: str, notes: str
) -> None:
    path = repo_root / "RELEASES.md"
    text = path.read_text(encoding="utf-8")
    if f"| {new_version} |" in text:
        raise PumpError(
            f"{path} already has a row for version {new_version} -- refusing to insert a duplicate."
        )
    separator = "|---|---|---|---|\n"
...
def main() -> int:
    ...
    parser.add_argument(
        "--notes", required=True, help="One-line RELEASES.md summary of this release"
    )
    parser.add_argument(
        "--allow-downgrade",
        action="store_true",
    ...
    try:
        ...
        old_occurrences_sh = (
            (repo_root / "install.sh").read_text(encoding="utf-8").count(old_version)
        )
        replace_exact(repo_root / "install.sh", old_version, new_version, old_occurrences_sh)

        old_occurrences_ps1 = (
            (repo_root / "install.ps1").read_text(encoding="utf-8").count(old_version)
        )
        replace_exact(repo_root / "install.ps1", old_version, new_version, old_occurrences_ps1)

        old_occurrences_readme = (
            (repo_root / "README.md").read_text(encoding="utf-8").count(old_version)
        )
        replace_exact(repo_root / "README.md", old_version, new_version, old_occurrences_readme)

        # The desktop app shares the CalVer: electron-updater reads its version
        # from package.json and looks for the same `v<version>` GitHub release
        # that carries the wheel, so the two must never drift.
        replace_exact(
    ...
    print("desktop/package.json, CHANGELOG.md, RELEASES.md, tests/test_release_consistency.py,")
    print("tests/test_install_scripts.py")
    print()
