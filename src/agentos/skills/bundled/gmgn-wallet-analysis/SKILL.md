...
description: >-
  Copy-trade dossier on one wallet — four pass/fail gates (is the record real, is the edge still working this week, can you actually get filled, does it cut losses), plus what it holds and buys right now, its entry market-cap band, its copy window in seconds, and a concrete size cap. Use when the user asks 「这个钱包能跟吗」, 「帮我分析一下这个钱包」, 「它现在在买什么」, 「这个钱包什么风格」, "can I copy this wallet", "analyze this wallet", "what is this wallet buying now", "what kind of trader is this", or pastes a bare wallet address — this is the default for a bare wallet address.
