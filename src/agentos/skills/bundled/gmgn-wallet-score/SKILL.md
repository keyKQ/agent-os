...
description: >-
  Score a wallet across three angles — track record (is this trader actually good?), copy-tradeability (can YOU capture it, plus a latency/slippage/gas backtest), and Dev reputation when the wallet mostly launches tokens — plus trading-style tags, computed deterministically from GMGN portfolio data. Use when the user asks 「钱包评分」, 「值不值得跟单」, 「这个钱包盈利能力怎么样」, 「dev 信誉怎么样」, "is this wallet profitable", "is this wallet worth following", "if I copy this wallet what's my real return", or "is this a token-creator wallet".
...
Run the following command, replacing the placeholders with the actual values. The last
four arguments are optional and default to `3.0` / `0.05` / `0.2` / `200` — pass them only
...
- FILL_IN_CHAIN: `sol` for base58 addresses; `bsc` for `0x...` unless the user names another chain
- FILL_IN_LANG: `zh` if the user wrote Chinese, `en` if English, default `zh`
- FILL_IN_LATENCY: seconds you would lag entering after this wallet (default `3.0`)
- FILL_IN_SLIPPAGE: one-sided slippage fraction, e.g. `0.05` = 5% (default `0.05`)
- FILL_IN_GAS: your cost per trade in USD (default `0.2`)
- FILL_IN_SAMPLE: activity rows to sample, max 400 (default `200`)
