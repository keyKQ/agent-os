...
#: Values that disclaim automation for the header carrying them. RFC 3834
#: defines ``Auto-Submitted: no`` for a message a human sent; Microsoft's
#: ``X-Auto-Response-Suppress: None`` likewise asks for *nothing* to be
#: suppressed, so an Outlook user whose mail carries it is not an
#: autoresponder. Presence alone would drop that mail silently, which is the
#: worse failure of the two: a loop is visible, a dropped question is not.
_NOT_AUTOMATED_VALUES: dict[str, frozenset[str]] = {
    "auto-submitted": frozenset({"no"}),
    "x-auto-response-suppress": frozenset({"none"}),
}
...
def is_automated(parsed: EmailMessage) -> bool:
    ...
    for header in _AUTOMATED_HEADERS:
        ...
        if not raw:
            continue
        if str(raw).strip().lower() in _NOT_AUTOMATED_VALUES.get(header, frozenset()):
            continue
        return True
