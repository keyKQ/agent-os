...
- **macOS installs through Homebrew.** This repository doubles as its own tap
  (`brew tap use-agent-os/agentos https://github.com/use-agent-os/agent-os`,
  `brew trust`, then `brew install --cask agentos`). The cask is generated by
  the release workflow from the built DMGs — version and per-architecture
  SHA-256 come from the artifacts themselves — and lands on `main` as
  `Casks/agentos.rb` through a self-merged pull request, so no cross-repo
  token exists. Because releases are not notarized, the cask clears the
  quarantine flag in a `postflight` block and says so as it runs; Homebrew
  removed `--no-quarantine` in 6.0.14, so this is the remaining route.

### Changed

- The bundled ONNX weights moved out of Git LFS into plain git objects. All
  three sit far under GitHub's blob limit, and the LFS filter is what made
  `brew tap` of this repository impossible — brew's scrubbed `PATH` has no
