...
from agentos.engine.cache_break_monitor import CacheBreakMonitor, PromptStateSnapshot
...
def _snapshot(monitor: CacheBreakMonitor) -> PromptStateSnapshot:
...
def test_reset_pending_is_bounded() -> None:
    ...
    assert len(monitor._baselines) < 5_000, "reset flags must be bounded"
...
def test_reset_pending_is_consumed_once() -> None:
    ...
    baseline = monitor._baselines.get("s1")
    assert baseline is not None
    assert baseline.reset_pending is False
...
def test_clear_drops_reset_flags() -> None:
    ...
    monitor.notify_compaction("s1")
    monitor.clear()
    assert "s1" not in monitor._baselines
